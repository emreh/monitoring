...
public final class InstrumentationInstaller {
    ...
    public static void install(Instrumentation inst, AgentConfig cfg) {
        ...
        MetricCollectorSingleton.get().init(cfg);
        ...
        try {
            if (cfg.instrumentation.executor.enabled) {
                builder = builder.type(isSubTypeOf(java.util.concurrent.ExecutorService.class)
                        ...
                        .transform((b, td, cl, md, pd) ->
                                b.method(named("execute").or(named("submit"))
                                        .or(named("invokeAll")).or(named("invokeAny")))
                                        .intercept(Advice.to(ExecutorServiceAdvice.class)));
            }

            if (cfg.instrumentation.jdbc.enabled) {
                builder = builder.type(nameStartsWith(cfg.rootPackage)
            ...
            if (cfg.instrumentation.timing != null && cfg.instrumentation.timing.enabled) {
                ...
                builder = builder.type((ElementMatcher<? super TypeDescription>) finalTypeMatcher)
                        .transform((b, td, cl, md, pd) -> {
                            var commonMatcher = isMethod().and(not(isConstructor())).and(not(isAbstract()))
